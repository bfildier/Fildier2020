...
def computePercentilesAndBinsFromRanks(sample,ranks,crop=True,vmin=0,vmax=float('inf')):
...
def definePercentilesOnLinQ(sample,n_bins):
	...
	ranks_linQ = getLinRanks(n_bins)

	ranks_linQ, centers_linQ, breaks_linQ = \
		computePercentilesAndBinsFromRanks(sample,ranks_linQ,crop=False)

	return ranks_linQ, centers_linQ, breaks_linQ

## Returns minimum and maximum values.
def findExtremumValues(sample,vmin=None,vmax=None,minmode='positive'):
...
def defineLinearBins(sample,n_lin_bins=nlb,vmin=None,vmax=None,minmode='positive'):
	...
	vmin,vmax = findExtremumValues(sample,vmin,vmax,minmode)
...
def computePercentileRanksFromBins(sample,centers):
	...
	n_pts = sample.size
	ranks = 100*np.array(list(map(lambda x:(sample < x).sum()/n_pts, centers)))
...
def ranksPercentilesAndBins(sample,mode='linear',n_pts_per_bin=nppb,
	n_bins_per_decade=nbpd,n_bins=100,vmin=None,vmax=None,crop=True,fill_last_decade=True,minmode='positive'):
	...
	if mode == 'linear':
		...
		ranks = getInvLogRanks(sample.size,n_pts_per_bin,n_bins_per_decade,
						fill_last_decade=fill_last_decade)
		ranks, percentiles, bins = computePercentilesAndBinsFromRanks(sample,
			ranks,crop=crop)
	elif mode == 'linQ':
		ranks, percentiles, bins = definePercentilesOnLinQ(sample,n_bins=n_bins)
	else:
...
def compute1dDensities(sample,mode='linear',n_pts_per_bin=nppb,\
	n_bins_per_decade=nbpd,n_lin_bins=nlb,vmin=None,vmax=None,minmode='positive'):
	...
	ranks, percentiles, bins = ranksPercentilesAndBins(sample,mode,n_pts_per_bin,\
		n_bins_per_decade,n_lin_bins,vmin,vmax,minmode=minmode)

	densities, edges = np.histogram(sample,bins=bins,density=True)

	return ranks, percentiles, bins, densities

## compute 1-CDF(X) from data and percentiles
def computeInvCDF(X,ranks,percentiles):
    
    out = np.ones(ranks.size)*np.nan
    for rank in ranks:
        iQ = indexOfRank(rank,ranks)
        XQ = percentiles[iQ]
        if not np.isnan(XQ):
            out[iQ] = np.nansum(X[X>XQ])/np.nansum(X)
    return out
...
def getRankLocations(rank,Y,ranks=None,bins=None,rank_locations=None,time_shift=0):

	"""time_shift: number of time steps to shift the mask in time (assuming it's the first dimension)"""
	...
	stencil_Q_shift = stencil_Q
	if time_shift != 0:
		stencil_Q_shift = np.roll(stencil_Q,shift=time_shift,axis=0)
		if time_shift < 0:
			stencil_Q_shift[time_shift:] = False
		else:
			stencil_Q_shift[:time_shift] = False
	# rank_locations[rank_id] = stencil_Q

	return stencil_Q_shift


#####---- Conditioning on percentile rank ----#####
...
def meanXProfileAtYRank(rank,X,Y,ranks,bins=None,rank_locations=None):
	...
	return np.array(computeTimeHorizontalMean(X,stencil_Q,is_3D=True))
...
def covMatXYAtZRank(rank,X1,X2,Y,ranks=None,bins=None,rank_locations=None,
	random_fraction=1):
...
## Froebenius inner product between two matrices of same dimensions
def prodFroeb(X,Y):
	return np.sum(X*Y)
